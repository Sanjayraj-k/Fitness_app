...
export default function LoginScreen() {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      const { createdSessionId, signIn, setActive } = await startGoogleOAuth();
      console.log('Google OAuth response:', { createdSessionId });
      if (createdSessionId) {
        ...
        try {
          ...
          Alert.alert('Warning', 'Failed to save user data, but login succeeded.');
        }
        navigation.reset({
          index: 0,
          routes: [{ name: 'HomeTab' }],
  ...
  const handleFacebookLogin = async () => {
    try {
      ...
      const { createdSessionId, signIn, setActive } = await startFacebookOAuth();
      console.log('Facebook OAuth response:', { createdSessionId });
      if (createdSessionId) {
        await setActive({ session: createdSessionId });
        const userData = {
  ...
  const handleAppleLogin = async () => {
    try {
      ...
      const { createdSessionId, signIn, setActive } = await startAppleOAuth();
      console.log('Apple OAuth response:', { createdSessionId });
      if (createdSessionId) {
        await setActive({ session: createdSessionId });
        const userData = {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      try {
        ...
        console.log('Firebase email user data stored:', userData);
      } catch (firestoreError) {
        console.error('Firestore write error:', firestoreError.message);
        Alert.alert('Warning', 'Failed to save user data, but login succeeded.');
      }
