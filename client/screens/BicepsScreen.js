...
const bicepWorkouts = [
  { name: 'Barbell Curl', calories: 110 },
  { name: 'Dumbbell Curl', calories: 100 },
  { name: 'Hammer Curl', calories: 100 },
  { name: 'Concentration Curl', calories: 90 },
  { name: 'Preacher Curl', calories: 95 },
];
  ...
  const fetchCompletedWorkouts = async (uid) => {
    try {
      ...
      querySnapshot.forEach((doc) => {
        const data = doc.data();
        if (bicepWorkouts.some((workout) => workout.name === data.type)) {
  ...
  return (
    <LinearGradient colors={['#6e45e2', '#88d3ce']} style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        <Text style={styles.header}>Biceps Workouts</Text>
        <View style={styles.workoutContainer}>
          {bicepWorkouts.map((workout, index) => (
